...
int main(int argc, char* argv[])
{
    ...
    if (MIN_ARG_VALUE > (uint) argc)
    {
        HIAI_ENGINE_LOG(HIAI_ARG_NUMBER_NOK, "Arg number is not right");
        return -1;
    }

    // concatenate test_source_file/test_dest_file/test_graph_file
    std::string model_name(argv[1]);
    TEST_SRC_FILE_PATH = "./test_data/" + model_name + "/data/src_test.bin";
    TEST_DEST_FILE_PATH = "./test_data/"+ model_name + "/result_file";
    GRAPH_CONFIG_FILE_PATH = "./test_data/" + model_name + "/config/graph_sample.prototxt";
    GRAPH_MODEL_PATH = "./test_data/" + model_name + "/model/ResNet-18-model.om";
